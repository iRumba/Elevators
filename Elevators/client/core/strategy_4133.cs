...
namespace client
{
    public class strategy : baseStrategy
    {
        ...
        {
            var free = Game.CurrentMyElevators.Where(el => el.State == (int)ElevStates.Filling);
            ...
            ManageInvites(Game.CanInvite.Keys);

            foreach(var elev in stand)
            {
                if (elev.HasInvitedPassengers())
                {
                    //WriteLog("Has Invited");
                    continue;
                }
                if (canMove.Contains(elev))
                {
                    var nf = elev.GoToNextFloor();
                    //if (elev.Floor == 9 && elev.GetAllPassengers().Count() == 0 && elev.IsReadyToMove() && !nf)
        ...
        void ManageInvites(IEnumerable<Elevator> elevators)
        {
            var floors = elevators.Select(elev => elev.Floor);
    ...
    public static class Game
    {
        ...
        public static int GetBestFloorWithPassengers(this Elevator elev)
        {
            ...
            var floorsWithPassengers = GetFloorsWithPassengers(elev).Where(f => !elev.GetAllPassengers().GetDestFloors().Contains(f.Floor));
            ...
            foreach(var item in floorsWithPassengers)
            {
                ...
                {
                    Floor = elev.Floor,
                    Passengers = item.ExpectedPassengers.OrderBy(ep => ep.Tick).Skip(rejected).Select(ep => ep.ToPart()),
                };
                ...
                foreach (var dest in byDest)
                {
                    ...
                    if (!dict.ContainsKey(dest.Key))
                        dict[dest.Key] = int.MinValue;
                    if (wCpt > dict[dest.Key])
                        dict[dest.Key] = wCpt;
                }
            }
        ...
        {
            return elev.GetAllPassengers().Count() == 0;
        }
        ...
        {
            return CurrentAllPassengers.Where(p => p.Elevator == elev.Id && p.State == 2);
        }

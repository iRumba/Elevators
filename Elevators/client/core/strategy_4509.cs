...
namespace client
{
    public class strategy : baseStrategy
    {
        ...
        void ManageInvites()
        {
            ...
            foreach (var f in floors)
            {
                ...
                var rejected = new List<Passenger>();
                ...
                foreach (var pwc in pwcCollection.OrderByDescending(pwc=>pwc.IncreasedCosts.Select(ic=>ic.CostWith - ic.CostWithout).Max()))
                {
                    ...
                    foreach (var ec in elevsCosts)
                    {
                        ...
                        if (elevFreeSpace <= 0)
                            continue;
                        var ordered = pwc.Passengers.OrderBy(p => p.IsMine()).ToList();
                        var toInv = ordered.Take(elevFreeSpace).ToList();
                        rejected.AddRange(ordered.Skip(elevFreeSpace));
                        toInv.ForEach(p => Game.InvitedPassengers[p] = ec.Elevator);
                    }
                }

                passengersOnFloor = rejected.GetByDestFloors();
                pwcCollection = new List<PassengersWithCostForElevators>();
                foreach (var pof in passengersOnFloor)
                {
                    var passengersWithCosts = new PassengersWithCostForElevators(passengersOnFloor[pof.Key]);
                    if (passengersWithCosts.IncreasedCosts.Any(ic => ic.CostWith > ic.CostWithout))
        ...
        private void ManageExpectedPassengers()
        {
            ...
            var forRemove = Game.ExpectedPassengers.Where(ep => ep.Id.HasValue && activePassengersIds.Contains(ep.Id.Value)).ToList();
            foreach(var fr in forRemove)
            {
                var diff = Game.Counter - fr.Tick;
                if (diff != 0)
                    WriteLog($"Move Type: {fr.MoveType} Dif: {diff}");
                //else
                //    WriteLog("ALL RIGHT!");
                Game.ExpectedPassengers.Remove(fr);
            }
    ...
    public static class Game
    {
        ...
        public static IEnumerable<FloorsWithPassengers> GetFloorsWithPassengers(this Elevator elev)
        {
            foreach (var floor in AllFloors)
            {
                ...
                var timeFor = Calculator.CalculateJumpTime(elev.Passengers, elev.Floor, floor);

                var tickWhen = timeFor + Counter;

                var waitings = PassengersOnFloors[floor].GetReady().Where(p => p.TimeToAway - (p.IsMine() ? elev.TimeForMyPassengerGoingTo() : elev.TimeForEnemyPassengerGoingTo() + Rules.MinStandingTimeForPickupEnemy) > timeFor + elev.GetTimeWhenAllPassengersWillInside());
        ...
        public static int GetBestFloorWithPassengers(this Elevator elev)
        {
            ...
            // Если куда то едет, то смотрим, есть ли смысл изменить маршрут.
            var preNext = elev.ComputeNextFloor();
            if (elev.IsFull())
                return preNext;

            // Получаем для данного лифта сводную информацию по пассажирам на этажах
            // Здесь этаж, пассажиры, которые дождутся этот лифт, пассажиры, которые появятся к его приезду,
            // время, за которое лифт доберется на этаж и будет готов к посадке
            // и пассажиры, которые появятся на этаже, где сейчас находится лифт, за это время
            var floorsWithPassengers = elev.GetFloorsWithPassengers();

            // Это количество потенциальных отказов при посадке
            // Вычисляется из свободных мест в более близких к месту посадки лифтах
            var rejected = GetPotentialRejects(elev);

            // Вычисляем информацию о текущем пути лифта (без учета промежуточных посадок)
            ...
            // Подготавливаемый словарь <Этаж, прирост прибыли>
            var dict = new Dictionary<int, float>();

            foreach(var item in floorsWithPassengers)
            {
                // Ожидающих пассажиров на этаже группируем по этажам назначения
                var waitingsByDest = item.WaitingPassengers.GetByDestFloors();

                //var expectedsByDest = item.ExpectedPassengers.GetAverageCost

                // Формируем посадку на лифт из ожидаемых на текущем этаже за то время, которое лифт ехал бы на этаж назначения
                ...
                // Создаем путь лифта с учетом последней посадки
                var eWay = new Way(elev, new Landing[] { eLand });

                // Считаем прибыль за тик для этого пути
                var eCpt = eWay.GetCostPerTick();

                var sum = 0f;

                // Перебираем все этажи в этажах назначения
                foreach (var dest in waitingsByDest)
                {
                    // Создаем посадку для пачки с одинаковыми этажами назначения
                    var wLand = new Landing
                    {
                        Floor = item.Floor,
                        Passengers = waitingsByDest[dest.Key].ToPart(),
                    };

                    // Создаем путь с учетом последней посадки
                    var wWay = new Way(elev, new Landing[] { wLand });

                    // Вычисляем прибыль за тик для этого пути и отнимаем от нее прибыль за тик для ожидания на текущем этаже
                    var wCpt = wWay.GetCostPerTick() - eCpt;

                    // Заполняем словарь, вычисляя на каком этаже пачка с максимальной прибылью
                ...
                dict[item.Floor] = sum / waitingsByDest.Count;
            }

            // Отбираем из словаря этажи, где прибыль плюсовая и выше чем текущая без посадки,
            // а так же сортируем по убиванию прироста прибыли
            ...
            {
                var other = CurrentAllElevators.Where(el => el != elev && el.NextFloor == floor && el.IsMoving() && el.GetTickWhenElevatorWillOnFloor(floor) < Calculator.CalculateJumpTime(elev.GetAllPassengers(), elev.Floor, floor) + Counter);

                // Если на этот этаж больше никто не приедет раньше нас, то возвращаем его
                if (other.Count() == 0)
                {
                    //if (floor == 1 && elev.Passengers.Count == 0 && floorsWithPassengers.First(fwp => fwp.Floor == floor).WaitingPassengers.Count==0)
                    //{
            ...
            if (preNext == -1 && elev.Floor == 1 && elev.Passengers.Count == 0 && Counter > 2000)
            {
                var sb = new StringBuilder();
                sb.AppendLine($"floorsWithPassengers COUNT: {floorsWithPassengers.Count()} RES COUNT: {res.Count()} DICT COUNT {dict.Count}");
                foreach (var item in floorsWithPassengers)
                {
                    sb.AppendLine($"Floor: {item.Floor} Waitings: {item.WaitingPassengers.Count} Expected: {item.ExpectedPassengers.Count} E OnFloor: {item.ExpectedsOnCurrent.Count}");
                }
                strategy.WriteLog(sb.ToString());
            }
            return preNext;
        }
        ...
        public static IEnumerable<int> GetExpectedPassengerFloors(this ExpectedPassengerInfo ep)
        {
            ...
            return AllFloors.Where(f => !VisitedFloors[ep.Id.Value].Contains(f));
        }
        ...
        //public static ILookup<int, ExpectedPassengerInfo> GetByDestFloors(this IEnumerable<ExpectedPassengerInfo> passengers)
        //{
        //    passengers.ToLookup(ep=>ep.GetAverageCost())
        //}
    ...
    public class Jump
    {
        ...
        public int Cost => Passengers.Where(p => p.DestFloor == ToFloor).GetCost();//.Sum(p => p.GetCost());
    }
    ...
    { 
        ...
        /// Конструктор
        /// </summary>
        /// <param name="elev">Лифт, который нужно проверять</param>
        /// <param name="landings">Набор посадок</param>
        ...
        public Way(IEnumerable<PartOfPassenger> passengers, int start, IEnumerable<Landing> landings = null)
        {
            ...
            var stop = passengers.FirstOrDefault() != null;
    ...
    public class ExpectedPassengerInfo
    {
        ...
        public int MoveType { get; internal set; }
    }

    /// <summary>
    /// Посадка (промежуточная)
    /// </summary>
    public class Landing
    {
        /// <summary>
        /// Пассажиры. Должна иметь одинаковый этаж
        /// </summary>
        public IEnumerable<PartOfPassenger> Passengers { get; set; }

        /// <summary>
        /// Этаж с которого совершается посадка
        /// </summary>
        public int Floor { get; set; }
    }
    ...
    public class PassengersWithCostForElevators
    {
        ...
        public PassengersWithCostForElevators(IEnumerable<Passenger> passengers)
        {
            ...
            {
                var forThisPassengers = passengers.Where(p => p.IsMine() || elev.CanPickupEnemy());
                var cWay = new Way(elev);
                var landing = new Landing
                {
                    Floor = Floor,
                    Passengers = forThisPassengers.ToPart(),
                };

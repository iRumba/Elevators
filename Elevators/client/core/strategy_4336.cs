        ...
        void ManageInvites()
        {
            ...
            foreach(var f in floors)
            {
                ...
                foreach(var pof in passengersOnFloor)
                {
                    ...
                    var elevsCosts = passengersWithCosts.IncreasedCosts.Where(ec => ec.IncreasedCost > 0).OrderByDescending(ec => ec.IncreasedCost);
                    foreach (var ec in elevsCosts)
                    {
                        var elevFreeSpace = ec.Elevator.FreeSpace() - ec.Elevator.InvitedCount();
                        if (elevFreeSpace >= passengersWithCosts.Passengers.Count)
                        {
                            passengersWithCosts.Passengers.ForEach(p => Game.InvitedPassengers[p] = ec.Elevator);
                            break;
                        }
                        var mine = passengersWithCosts.Passengers.Where(p => p.IsMine()).ToList();
    ...
    public class PassengersWithCostForElevators
    {
        ...
        public PassengersWithCostForElevators(IEnumerable<Passenger> passengers)
        {
            ...
            foreach(var elev in elevsOnFloor)
            {
                ...
                {
                    Elevator = elev,
                    IncreasedCost = wWay.GetCostPerTick() - wWay.GetCostPerTick(),
                };

    ...
    public static class Game
    {
        ...
        public static IEnumerable<FloorsWithPassengers> GetFloorsWithPassengers(this Elevator elev)
        {
            ...
            foreach (var floor in AllFloors)
            {
                ...
                var timeWhen = Calculator.CalculateJumpTime(elev.GetAllPassengers(), elev.Floor, floor);

                var tickWhen = timeWhen + Counter;

                var waitings = PassengersOnFloors[floor].GetReady().Where(p => p.TimeToAway - (p.IsMine() ? elev.TimeForMyPassengerGoingTo() : elev.TimeForEnemyPassengerGoingTo() + Rules.MinStandingTimeForPickupEnemy) > timeWhen + elev.GetTimeWhenAllPassengersWillInside());
        ...
        public static int GetBestFloorWithPassengers(this Elevator elev)
        {
            ...
            foreach(var item in floorsWithPassengers)
            {
                ...
                dict[item.Floor] = sum / byDest.Count;
            }
            ...
            {
                strategy.WriteLog($"floorsWithPassengers COUNT: {floorsWithPassengers.Count()} RES COUNT: {res.Count()}");
            }
